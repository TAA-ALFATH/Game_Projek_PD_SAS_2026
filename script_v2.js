...
function render() {
  ...
  for (let y=0;y<ROWS;y++){
    for (let x=0;x<COLS;x++){
      ...
      if (enemy) cell.classList.add(enemy.boss? 'boss':'enemy');
...
function placeBomb(){
  ...
  player.score+=5; updateUI(); playSfx(placeBombSfx,0.45); render();
}
  ...
  enemies.forEach(enemy => {
    if (explosions.some(e=>e.x===enemy.x&&e.y===enemy.y)){
      if (enemy.boss){
        ...
        if (enemy.hp <= 0){ enemy.alive = false; player.score += 500; playSfx(deadSfx, 1.0, 0.75); }
      } else {
        enemy.alive = false;
        player.score += 100;
...
function moveEnemies(){ if (gameOver) return; enemies.forEach(enemy=>{ if (!enemy.alive) return; const dirs=[{dx:1,dy:0},{dx:-1,dy:0},{dx:0,dy:1},{dx:0,dy:-1}]; const valid=dirs.filter(d=>{ const nx=enemy.x+d.dx, ny=enemy.y+d.dy; return nx>=0&&nx<COLS&&ny>=0&&ny<ROWS && map[ny][nx]!==WALL && map[ny][nx]!==SOFT_WALL && !bombs.some(b=>b.x===nx&&b.y===ny); }); if (valid.length>0){ const d=valid[Math.floor(Math.random()*valid.length)]; enemy.x+=d.dx; enemy.y+=d.dy; } if (enemy.x===player.x&&enemy.y===player.y) playerHit(); }); render(); checkLevelComplete(); }
